...
import com.min.app09.utill.FileUtil;

import lombok.RequiredArgsConstructor;

@RequiredArgsConstructor
@Service  // 컨트롤러(NoticeController)가 서비스를 사용기위해선 서비스 bean을 주입하여 사용하여 에노테이션을 붙임// @Component를 써도 동일하게 작동함
public class NoticeServiceImpl implements INoticeService {
  ...
  public String registNotice(MultipartHttpServletRequest multipartRequest) {
    ...
    // DB에 공지사항 등록하기
    int result = noticeDao.insertNotice(noticeDto); // mapper의 <selectKey>에 의해서 noticeDtodml noticeId 필드에 값이 채워집니다.
    if(result == 0)
      return "공지사항 등록 실패";
  ...
  public String removeNotice(int noticeId) {
    ...
    return result == 1 ? "공지사항 삭제성공" : "공지사항  삭제 실패";
  }
  
  @Override
  public ResponseEntity<Resource> download(int attachId, String userAgent) { // String userAgent 어떤 브라우저에서 접속하는지 값 받아오기
    ...
    // 다운로드 할 첨부파일의 이름 정하기 (originalFilename 기준, 브라우저마다 다른 인코딩 적용)(확인방법 : 크롬,웨일 f12->콘솔 -> navigator.userAgent)
    ...
    HttpHeaders responseHeader = new HttpHeaders();  // HttpHeaders : org.springframework.http.HttpHeaders
    try {
      ...
      responseHeader.add("content-length", resource.contentLength() + "");                    // 응답 데이터의 길이를 의미합니다. 바이트 단위로 작성합니다.
                                                                                              // reponseHeader.add(String, String) 이라서 +"" 빈문자열 넣어줌
    } catch (Exception e) {
      e.printStackTrace();
    }
