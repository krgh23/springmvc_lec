...
<body>
  ...
  <div>
    <form action="${contextPath}/notice/regist.do" method="POST" enctype="multipart/form-data">
      ...
      <input type="file" name="files" id="files" multiple><br/> <!-- multiple="multiple" 이름과 내용이 같은경우 생략가능-->
  ...
  <script>
    function attachCheck() {
      ...
      files.addEventListener('change', (event) => {
        ...
        for(const file of event.currentTarget.files) {  // files 객체(event.currentTarget)의 files 프로퍼티 // for(const file of files.files)
          if(file.size > limitPerSize) {
            alert('각 첨부 파일의 크기는 최대 10MB 입니다.');
