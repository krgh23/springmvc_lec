...
public class BoardController {
  ...
  public String list(HttpServletRequest request, Model model) {   // request : 요청, model : 응답
  ...
  public String modify(BoardDto boardDto, RedirectAttributes redirectAttributes) {
    // 수정 서비스로부터 수정 결과를 받아온 뒤, 해당 정보를 리다이렉트 할 수 있드록 redirectAttributes에 저장합니다.
    ...
    return "redirect:/board/detail.do?boardId={id}";  // 상세보기로 리다이렉트(상세보기 요청을 작성합니다.)
                                                      // {id}는 attribute로 저장된 id를 의미합니다. flesh attribute가 아닌 그냥 attribute이다.
  }
  
  @RequestMapping(value = "/remove.do", method=RequestMethod.POST)
  public String remove(int boardId, RedirectAttributes redirectAttributes) { // @RequestParam 생략
  ...
  public String removes(String[] numbers, RedirectAttributes redirectAttributes) { // @RequestParam 생략
  ...
  public String register(BoardDto boardDto, RedirectAttributes redirectAttributes) { 
    // 삽입 서비스로부터 삽입 결과를 받아온 뒤, 해당 정보를 리다이렉트 할 수 있도록 redirectAttributes에 저장합니다
