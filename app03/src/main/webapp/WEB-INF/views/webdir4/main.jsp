...
<body>
  ...
  <%-- 
    ...
    method
      ...
      3) 디폴트는 get 방식입니다.
      4) 모든 요청을 Query String 방식의 요청으로 처리됩니다.
      5) get 방식으로 처리하면 요청 주소에 요청 파라미터가 노출됩니다. (빠르게 동작합니다.)
      6) post 방식으로 처리하면 요청 주소에 요청 파라미터가 노출되지 않는다. (보안에 좋습니다. 대신 약간 느립니다.)
  --%>

  <form action="<%=request.getContextPath()%>/webdir4/req1" method="get">
    <%-- 입력상자(text, number, email, url, password, date) --%>
    ...
    <%-- 서브밋 action에 작성된 주소로 모든입력 요소들를 보냅니다.  --%>    
  ...
  <%-- 모든 선택 상자는 선택했을 경우 디폴트 value = "on"을 가집니다. --%>
  
  <%-- 선택 상자 : 다중 선택이 가능한 chechbox (같은 name 으로 보내기) - 디폴트 value="on"을 사용할 수 없습니다. --%>
  ...
  <%-- 선택 상자 : 다중 선택이 가능한 chechbox (다른 name 으로 보내기) - 디폴트 value="on"을 사용할 수 있습니다. --%>
  ...
  <%-- 선택 상자 : 단일 선택만 가능한 readio - 디폴트 value="on"을 사용할 수 없습니다. --%>
  ...
  <form action "<%=request.getContextPath()%>/webdir4/req3" method="get">
    ...
    <%-- 모든 목록 요소는 <option> 태그의 내부텍스트(textContent)가 value 로 전달됩니다. --%>
    <%-- <option> 태그에 value 속성을 추가하면 value 속성 값이 전달됩니다. --%>
  
    <%-- 목록요소 : <select> 태그와 <option> 태그 --%>
