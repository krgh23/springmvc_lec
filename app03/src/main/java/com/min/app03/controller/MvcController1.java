...
public class MvcController1 {
  ...
  /*
   * 반환타입
   *   1)String       : 사용자에게 보여 줄 (응답할) JSP 의 경로와 이름을 작성합나다.
   *   2)void         : 요청한 주소를 JSP 경로와 이름으로 인식합니다.
   *   3)ModelAndView : 사용자에게 보여 줄 JSP의 경로와 이름은 setViewName() 메소드로 저장하고, 전달할 값은 addObject() 메소드로 저장합니다.
   *   4)...
   *  
   * 메소드명
   *   아무런 의미가 없다.
   ...
   *     (4) HttpSession 인터페이스         : 세션을 처리 할 수 있는 인터페이스 입니다.
   */     
   ...
   * 2. 요청 주소 (value)
   *   1) value="/"                   : Context Path 요청을 의미한다. http://127.0.0.1:8080/app03 (대표주소로 접속하면 보여줄 페이지)
   *   2) value="/lest"               : /list 요청을 의미한다.        http://127.0.0.1:8080/app03/list
   *   3) value={"/lest", "/lest.do"} : 2개이상의 요청을 의미한다     http://127.0.0.1:8080/app03/list
  ...
  public String welcome() {
     ...
     * 
     * 1. 리턴한 값 "webdir1/index"는 ViewResolver 에게 전달 됩니다.
  ...
  @RequestMapping(value="/webdir1/list", method=RequestMethod.GET) //실행하면 위 welcome 이 나옴으로 주소를 바꿔서 확인필요
  public void letsgo() {
    // 반환타입이 void 이므로 요청 주소 "/webdir1/list"를 사용자에게 보여 줄 JSP 경로와 이름으로 해석합니다.
    // ViewResolver 에 의해서 최종 경로는 "/WEB-INF/views/webdir1/list.jsp" (주소를 넣는대 폴더 구조가 보인다)
  }
  
  @RequestMapping(value="/board/view")  // method 를 생략하면 get 방식으로 동작합니다(디폴트)
  public ModelAndView gogogo() {
    ModelAndView mav = new ModelAndView();
    mav.setViewName("webdir1/detail"); // 사용자에게 보여줄 JSP의 경로와 이름입니다. ViewResolver 에 의해 처리됩니다.
