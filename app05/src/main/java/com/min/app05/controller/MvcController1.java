...
// @Controller   // MvcController2를 실행할때 MvcController1이 작동하지 않도록 주석처리함
public class MvcController1 {
  ...
  public String login(
    ...
    // 세션 아이디를 확인해 봅니다. (브라우저의 쿠키 : F12 -> Application -> Cookies 에서 JSESSIONID 쿠키로 사용자에게 전달달되어 저장됩니다.
    // System.out.println(session.getId());
    
    // 세션 유효 시간을 설정해 봅니다. (초 단위로 설정합니다. 시간이 지나면 새로운 세션을 발급받습니다.)
    // 세션의 기본 유효시간은 1800초 입니다. (1800초 = 30분)
    // session.setMaxInactiveInterval(5); // 5초 후에 소멸되는 세션
    
    // id와 pw를 이용해서 UserVo 인스턴스를 만듭니다
    ...
    if(id.equals(pw)) {
      ...
      // 세션에 저장할 때도 속성(Attribue)로 저장합니다.
